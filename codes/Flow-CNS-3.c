...
int main(int argc, char *argv[])
{
    ...
    for(n=0; n<evel; n++)
    {
        for(m=0; m<evel; m++)
        {
            ...
            mpfr_mul(temp, temp2, FOUR, GMP_RNDN);
            mpfr_add(temp, temp, temp1, GMP_RNDN);
            mpfr_sin(temp, temp, GMP_RNDN);
            ...
            mpfr_mul(temp, temp2, FOUR, GMP_RNDN);
            mpfr_sub(temp, temp, temp1, GMP_RNDN);
            mpfr_sin(temp, temp, GMP_RNDN);
            mpfr_mul(temp, temp, Ssmall, GMP_RNDN);
            ...
            {
                {
                    ...
                    for(k=0; k<N; k++)
                    {
                        for(j=0; j<N; j++)
                        {
                            mpfr_sprintf(Char1, "%.432Re", CoeALL[NOWid[j][k]][NOWm[j][k]][NOWn[j][k]]);
                            mpfr_sprintf(Char2, "%.432Re", CoeALL[NOWid[j][k]][NOWm[j][k]][NOWn[j][k]+evel]);
                            fprintf(fp, "%d\t%d\t%.442s\t%.442s\n", k, j, Char1, Char2);
                        }
                    }
                    fclose(fp);
                }
